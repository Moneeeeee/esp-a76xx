...
bool Ml307AtModem::WriteRaw(const std::string& data) {
    std::lock_guard<std::mutex> lock(mutex_);
    int ret = uart_write_bytes(uart_num_, data.c_str(), data.size());
    if (ret < 0 || static_cast<size_t>(ret) != data.size()) {
        ESP_LOGE(TAG, "uart_write_bytes failed in WriteRaw");
        return false;
    }
    return true;
}

EventGroupHandle_t Ml307AtModem::GetEventGroupHandle() const {
    return event_group_handle_;
}



bool Ml307AtModem::CommandWaitPrompt(const std::string& command, char prompt_char, int timeout_ms) {
    std::lock_guard<std::mutex> lock(command_mutex_);
    if (debug_) {
    ...
    while ((esp_timer_get_time() - start) < timeout_ms * 1000LL) {
        std::lock_guard<std::mutex> lock(mutex_);
        auto pos = rx_buffer_.find(prompt_char);
        if (pos != std::string::npos) {
            rx_buffer_.erase(0, pos + 1);
            ESP_LOGI(TAG, "Received prompt: %c", prompt_char);
            return true;
        }
        vTaskDelay(pdMS_TO_TICKS(10));
    }

    ESP_LOGE(TAG, "Timeout waiting for prompt: %c", prompt_char);
    return false;
}
    ...
    if (rx_buffer_[0] == '+') {
        ...
        while (std::getline(iss, item, ',')) {
            ...
            if (item.size() >= 2 && item.front() == '"' && item.back() == '"') {
                ...
                // 默认 fallback 为字符串（如 IP 地址 10.0.0.123）
                argument.type = AtArgumentValue::Type::String;
                argument.string_value = item;
            }
        ...
        xEventGroupSetBits(event_group_handle_, AT_EVENT_COMMAND_PROMPT);  // 改为专用事件
        xEventGroupSetBits(event_group_handle_, AT_EVENT_COMMAND_DONE);
        return true;
...
void Ml307AtModem::NotifyCommandResponse(const std::string& command, const std::vector<AtArgumentValue>& arguments) {
    ...
    if (command == "CGPADDR" && arguments.size() >= 2) {
        if (arguments[0].int_value == 1) {
            const std::string& ip = arguments[1].string_value;
            if (!ip.empty() && ip != "0.0.0.0") {
                ip_address_ = ip;
                ESP_LOGI(TAG, "✅ Got valid IP: %s", ip.c_str());
                network_ready_ = true;
                xEventGroupSetBits(event_group_handle_, AT_EVENT_NETWORK_READY);
            } else {
                ESP_LOGW(TAG, "⚠️ Invalid IP from CGPADDR: %s", ip.c_str());
            }
        }
        ...
        registration_state_ = (arguments.size() == 1) ? arguments[0].int_value : arguments[1].int_value;
    } else if (command == "CPIN" && arguments.size() >= 1) {
        pin_ready_ = (arguments[0].string_value == "READY") ? 1 : 2;
    }
...
void Ml307AtModem::ResetConnections() {
    ...
    Command("AT+HTTPTERM");//关闭HTTP Service
    // Command("AT+NETCLOSE");//关闭Socket Service
    Command("AT+CMQTTDISC=0,60");// 断开连接
    Command("AT+CMQTTREL=0");// 释放上下文
    Command("AT+CMQTTSTOP");// 停止 MQTT 服务（释放资源）
